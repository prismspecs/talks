...
Society has reached peak awareness. Producing evidence of broken systems is no longer enough. The goal here is to move past documentation and toward direct intervention on the systems that cause harm, blurring the line between conceptual art and functional sabotage.
...
The power of distributed computing and active participation. Launched in 1999 because Berkeley couldn't afford supercomputer time to analyze satellite data from the Puerto Rico array. Distributing raw data to volunteers turned their idle machines into an ad hoc supercomputer.

> "It's not really like volunteering or donating. It's another step. You are doing the thing itself as well with them. It transforms you into a more invested participant."

It proved that a collective goal could be reached using idle computing power alone, a shift from passive charity or volunteering to direct material participation. Users are doing the thing itself.

---

## Bail Bloc {#bail-bloc}

Distributed participation turned toward abolitionist sabotage. A background desktop app, inspired by SETI@home, that uses idle processing power (roughly what a Netflix stream takes) to mine Monero. The crypto is converted to USD to chip away at the carceral state's financial leverage.

Partnered with the Bronx Freedom Fund and the Connecticut Immigrant Bond Fund to release people, including those detained by ICE.
...
## 79% Work Clock (Disobedient Electronics, PARTY) {#work-clock}

Physicalizing systemic inequity. Made for Equal Pay Day, this physical alarm clock alerts women when 79% of the workday has passed, reflecting the wage gap. It proposes a direct action: if compensation is only 79%, labor output should stop at exactly that threshold.

---

## Transparency Grenade (Julian Oliver, 2012) {#transparency-grenade}

Forced transparency. A device shaped like a hand grenade that captures network traffic and audio from the surrounding environment and securely leaks it to a public server, a tool for corporate or governmental whistleblowing.

---

## Reflecto Cube / Protest Cube (Colectivo Enmedio / Tools for Action) {#reflecto-cube}

Defensive tactical architecture. DIY giant silver inflatable cubes used in demonstrations to stop police riot charges, both aesthetically striking and genuinely practical. They physically block police lines, buffer against batons, and reflect police lights and flashes back at authorities to cause momentary vision loss. Re-inflatable and reusable.
